...
function initializeEventListeners() {
    ...
    birthdayTitle.addEventListener('click', () => {
        ...
        confetti({
            ...
            spread: 100,
            origin: { y: 0.4 },
            colors: ['#0a192f', '#FFD700', '#C0C0C0', '#ffffff', '#3b82f6'] // Police Blue, Gold, Silver, White, Blue
        });
...
async function handleFormSubmit(e) {
    ...
    try {
        ...
        if (file) {
            // Validate file size (25MB max - matches Firebase Storage rules)
            ...
            if (file.name.toLowerCase().endsWith('.heic') || file.type === 'image/heic') {
                progressText.textContent = 'Converting HEIC image... (this may take a moment)';
                ...
                try {
                    ...
                    // heic2any can return a Blob or an array of Blobs. We want just one.
                    const finalBlob = Array.isArray(convertedBlob) ? convertedBlob[0] : convertedBlob;

                    // Create a new File object
                    fileToUpload = new File([finalBlob], file.name.replace(/\.heic$/i, ".jpg"), {
                        type: "image/jpeg"
                    });
                    console.log("Conversion successful!");
                } catch (e) {
                    console.error("HEIC conversion failed:", e);
                    showToast("Could not convert HEIC image. Try a JPG.", "error");
            ...
            const uploadResult = await uploadFile(fileToUpload, (progress) => {
                // Update progress bar
